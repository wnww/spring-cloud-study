...
public class TransactionIntercept {
		...
		if (t != null && this.checkAnnotationValue(t.value())) {
			try {
				if(t.value().equals(Constants.Type.ACTIVITY)){
					...
					activityId = this.saveTxTransaction(t, className, classMethod.getName(), classMethod.getParameterTypes(), args);
	 ...
	 * @return
	 */
	private String saveTxTransaction(TxTransaction t, String className, String methodName,Class<?>[] paramterTypes,Object[] args) throws Exception{
	 ...
	 * @param paramters
	 */
	private String saveActivity(TxTransaction t,String methodName, String callUrl, String className, Class<?>[] paramterTypes,Object[] args){
	 ...
	 * @param parameters
	 */
	private void saveAction(TxTransaction t,String methodName, String callUrl, String className, Class<?>[] paramterTypes,Object[] args) throws Exception{
	...
	private void callConfirm(TxTransaction t, ProceedingJoinPoint pjp, String activityId){
		...
		if(exists){
			...
			remoteCallService.confirmService(activity.getCallConfirmUrl(), activity.getParameters());
			List<Action> actions = activity.getActions();
			for(Action action : actions){
				remoteCallService.confirmService(action.getCallConfirmUrl(), action.getParameters());
			}
		}
	...
	private void callCancel(TxTransaction t, ProceedingJoinPoint pjp, String activityId){
		...
		if(exists){
			...
			List<Action> actions = activity.getActions();
			for(Action action : actions){
				remoteCallService.cancelService(action.getCallCancelUrl(), action.getParameters());
			}
		}
